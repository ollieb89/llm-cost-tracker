...
export function parseLogContent(content: string): ParsedUsage[] {
  ...
  while ((m = genRe.exec(content)) !== null) {
    ...
    if (provider === 'unknown' || !lookupPricing(provider, model)) continue;
...
export function validateTokens(inputTokens: number, outputTokens: number): string | null {
  if (!Number.isInteger(inputTokens) || inputTokens < 0) return 'input-tokens must be a non-negative integer';
  if (!Number.isInteger(outputTokens) || outputTokens < 0) return 'output-tokens must be a non-negative integer';
  if (inputTokens === 0 && outputTokens === 0) return 'both input-tokens and output-tokens are zero';
  if (inputTokens > 10_000_000) return 'input-tokens exceeds 10M — verify this is correct';
  if (outputTokens > 2_000_000) return 'output-tokens exceeds 2M — verify this is correct';
  return null;
}
