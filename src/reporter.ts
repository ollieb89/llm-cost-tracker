...
export function formatMarkdownReport(report: CostReport, workflowName?: string): string {
  ...
  lines.push('|--------|-------|');
  lines.push(`| Total Cost | **${formatUSD(totalCost)}** |`);
  ...
  if (budgetStatus.budgetLimit !== null) {
    lines.push(`| Budget Limit | ${formatUSD(budgetStatus.budgetLimit)} |`);
    lines.push(`| Budget Remaining | ${formatUSD(Math.max(0, budgetStatus.budgetRemaining ?? 0))} |`);
  ...
  if (records.length > 0) {
    ...
    for (const r of records) {
      lines.push(`| ${r.provider} | ${r.model} | ${r.inputTokens.toLocaleString()} | ${r.outputTokens.toLocaleString()} | ${formatUSD(r.totalCost)} |`);
    }
    lines.push('');
  }
  ...
  if (budgetStatus.budgetExceeded) {
    ...
    lines.push('');
    lines.push(`This run cost **${formatUSD(totalCost)}**, exceeding the budget of **${formatUSD(budgetStatus.budgetLimit ?? 0)}** by **${formatUSD(budgetStatus.overrunAmount)}** (${budgetStatus.overrunPercent}% over).`);
    lines.push('');
    ...
    lines.push('');
    lines.push(`This run cost **${formatUSD(totalCost)}** of **${formatUSD(budgetStatus.budgetLimit)}** budget.`);
    lines.push('');
  }
...
export function formatJobSummary(report: CostReport): string {
  return formatMarkdownReport(report);
}

export function formatTextSummary(report: CostReport): string {
  const lines = [
    'LLM Cost Tracker',
    '================',
    `Total cost:    ${formatUSD(report.totalCost)}`,
  ...
  if (report.budgetStatus.budgetLimit !== null) {
    lines.push(`Budget limit:  ${formatUSD(report.budgetStatus.budgetLimit)}`);
    lines.push(`Budget left:   ${formatUSD(Math.max(0, report.budgetStatus.budgetRemaining ?? 0))}`);
  ...
  if (report.records.length > 0) {
    ...
    for (const r of report.records) {
      lines.push(`  ${r.provider}/${r.model}: ${(r.inputTokens + r.outputTokens).toLocaleString()} tokens = ${formatUSD(r.totalCost)}`);
    }
  }
  return lines.join('\n');
}

export function recordsToCSV(records: CostRecord[]): string {
  ...
  const rows = records.map(r =>
    [r.timestamp, r.provider, r.model, r.inputTokens, r.outputTokens, r.totalTokens,
